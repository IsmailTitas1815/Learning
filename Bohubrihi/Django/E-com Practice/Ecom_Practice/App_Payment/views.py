...
from App_Payment.forms import CheckoutAddress
...
def checkout(request):
    ...
    form = CheckoutAddress(instance=saved_address)
    if request.method == 'POST':
        form = CheckoutAddress(request.POST, instance=saved_address)
        if form.is_valid():
            form.save()
            form = CheckoutAddress(instance=saved_address)
...
def complete(request):
    if request.method == "POST" or request.method =="post":
        ...
        if status == "VALID":
            ...
            messages.success(request,f"Your Payment Completed successfully! Page will be redirected to home after 5 seconds.")
            return HttpResponseRedirect(reverse("App_Payment:purchase", kwargs={"val_id":val_id, 'tran_id':tran_id}))
        elif status == "FAILED":
            messages.success(
                request, f"Your Payment failed! Please try again! Page will be redirected to home after 5 seconds.")
